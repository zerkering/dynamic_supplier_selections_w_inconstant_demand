...
%mul = Aeq(25,1:376)* x(1:376)
ceq = -1*x(69) +x(93) -(5000*((exp(-(-6 + 0.015*x(377))))/(1+(exp(-(-6 + 0.015*x(377)))))));
c = [];
end

nonlcon = @circlecon;

options = optimoptions('fmincon','MaxIterations',10^4, 'MaxFunctionEvaluations',10^8);
